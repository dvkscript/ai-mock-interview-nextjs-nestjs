...
import { cn, StatusBgColor, StatusText } from "@/lib/utils";
...
const RoomDetailClient: React.FC<RoomDetailClientProps> = ({
    ...
    const maxQuestionIndex = useMemo(() => {
        return Math.max(...jobs.questions.map(obj => obj.index));
    }, [jobs.questions]);

    const messageLast = useMemo(() => {
    ...
    useEffect(() => {
        if (jobs.progressAnswer === 0) return;
    ...
    useEffect(() => {
        if (!messageLast || jobs.status !== JobStatus.IN_PROGRESS && jobs.status !== JobStatus.NOT_STARTED) return;

        if (messageLast.role !== "assistant" || messageLast.index < maxQuestionIndex) return;
    ...
    return (
        <ClientOnly>
            ...
            <Dialog open={showFeedbackModal} onOpenChange={setShowFeedbackModal}>
                <DialogContent className="sm:max-w-[425px] p-0 overflow-hidden" aria-describedby="feedback-modal">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                            ...
                            <div className="absolute bottom-6 right-6 w-32 h-32 rounded-2xl overflow-hidden border-2 border-white dark:border-gray-700 shadow-lg transition-all hover:scale-105">
                                <div className="w-full h-full bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center">
                                    {isVideoOff ? (
                                        ...
                                        <Avatar
                                            className="w-20 h-20 border-2 border-white dark:border-gray-600"
                                        >
                                            <AvatarImage src={profile?.thumbnail} alt={profile?.fullName.charAt(0)} />
                                            <AvatarFallback
                                                className="bg-gradient-to-br from-blue-500 to-indigo-500 text-white text-xl font-bold"
                                            >
                            ...
                            <div className="absolute top-4 right-4 px-4 py-2 rounded-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm text-gray-700 dark:text-gray-300 shadow-md border border-gray-200 dark:border-gray-700">
                                <span className="text-sm font-medium flex items-center">
                                    <span className={cn(
                                        ...
                                        StatusBgColor[jobs.status === JobStatus.COMPLETED ? JobStatus.COMPLETED : JobStatus.IN_PROGRESS],
                                    )}></span>
                                    {StatusText[jobs.status === JobStatus.COMPLETED ? JobStatus.COMPLETED : JobStatus.IN_PROGRESS]}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-center gap-6 p-6 bg-gradient-to-r from-gray-50 to-white dark:from-gray-850 dark:to-gray-800 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <Button
                                ...
                                disabled={JobStatus.COMPLETED === jobs.status}
