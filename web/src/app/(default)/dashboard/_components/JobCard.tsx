"use client"
import { GetJobAndCountAll, jobReStarted, reGenerateQuestion } from "@/actions/job.action";
...
const JobCard: React.FC<JobCardProps> = ({
    ...
    const handleReStarted = (job: GetJobAndCountAll["rows"][number]) => {
        startTransition(async () => {
            const res = await jobReStarted(job.id);
            
            if (!res.ok) {
                toast.error(res.message);
            } else {
                router.push(`/interview/${job.id}/room`)
            }
        })
    }
        ...
        if (job.status === JobStatus.COMPLETED) {
            return (
                <>
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 mr-1.5 ${isPending ? "animate-spin" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div>
                    ...
                    {job.status === JobStatus.COMPLETED && (
                        <div className="mt-3">
                            <Link
                                href={`/interview/${job.id}/room`}
                                className="flex-1 inline-flex w-full justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 transition-colors shadow-sm"
                            >
